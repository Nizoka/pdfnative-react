...
Sixteen rules, each with a stable code. Branch on the code, not the message.

### Errors — these clear `ok`

| Code | Rule |
|---|---|
| `L_EMPTY_DOCUMENT` | The document has no blocks |
| `L_TAGGED_NO_FONTS` | PDF/A requested with no `fontEntries` (veraPDF 6.2.11.4.1) |
| `L_TAGGED_ENCRYPTED` | PDF/A and encryption combined (ISO 19005-1 §6.3.2) |
| `L_ATTACHMENTS_NEED_PDFA3` | Attachments outside `tagged="pdfa3b"` (ISO 19005-3) |
| `L_CHART_SERIES` | Pie or donut with anything other than one series |
| `L_CHART_CATEGORIES` | Series length ≠ `categories.length` |
| `L_CHART_VALUES` | Non-finite value, or a negative in a pie/donut |
| `L_CHART_POINTS` | Chart past the engine's 10 000-point ceiling |

The last five would each throw at render time.
...
| `L_HEADING_HIERARCHY` | Heading level skipped (h1 → h3) |
